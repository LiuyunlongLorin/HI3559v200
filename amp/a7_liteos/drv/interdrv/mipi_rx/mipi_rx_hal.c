...
extern void serdes_reset_unreset_sensor(int reset);
extern void serdes_enable_disable_sensor_clock(int enable);
#endif
...
#ifdef HI_SERDES_SUPPORT
    serdes_enable_disable_sensor_clock(enable);
#endif
}
...
#ifdef HI_SERDES_SUPPORT
    serdes_reset_unreset_sensor(reset);
#endif
}
...
int mipi_rx_drv_init(void)
{
    ...
    mipi_rx_core_reset_addr  = (unsigned long)osal_ioremap(MIPI_RX_CRG_ADDR, (unsigned long)0x4);
