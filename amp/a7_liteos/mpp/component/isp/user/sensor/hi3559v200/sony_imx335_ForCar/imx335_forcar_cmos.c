 ...
 ******************************************************************************
  File Name     : IMX335_forcar_cmos.c
...
******************************************************************************/
#if !defined(__IMX335_FORCAR_CMOS_H_)
...
#include "imx335_forcar_cmos_ex.h"
#ifdef __cplusplus
#if __cplusplus
extern "C" {
#endif
...
#define IMX335_FORCAR_HIG_4BITS(x) (((x) & 0xF0000) >> 16)
#define IMX335_FORCAR_MID_8BITS(x) (((x) & 0x0FF00) >> 8)
#define IMX335_FORCAR_LOW_8BITS(x)  ((x) & 0x000ff)
...
ISP_SNS_STATE_S *g_pastImx335_forcar[ISP_MAX_PIPE_NUM] = {HI_NULL};

#define IMX335_FORCAR_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastImx335_forcar[dev])
#define IMX335_FORCAR_SENSOR_SET_CTX(dev, pstCtx)   (g_pastImx335_forcar[dev] = pstCtx)
#define IMX335_FORCAR_SENSOR_RESET_CTX(dev)         (g_pastImx335_forcar[dev] = HI_NULL)
...
ISP_SNS_COMMBUS_U g_aunImx335_forcar_BusInfo[ISP_MAX_PIPE_NUM] =
{
    [0] = { .s8I2cDev = 0},
...
typedef struct hiIMX335_FORCAR_STATE_S
{
    HI_U32      u32BRL;
    HI_U32      u32RHS1_MAX;
    HI_U32      u32deltaRHS1;
} IMX335_FORCAR_STATE_S;

IMX335_FORCAR_STATE_S g_astimx335_forcar_State[ISP_MAX_PIPE_NUM] = {{0}};
 ...
 * extern                                                                   *
 ****************************************************************************/
extern unsigned char imx335_forcar_i2c_addr;
extern unsigned int  imx335_forcar_addr_byte;
extern unsigned int  imx335_forcar_data_byte;

extern void IMX335_forcar_init(VI_PIPE ViPipe);
extern void IMX335_forcar_exit(VI_PIPE ViPipe);
extern void IMX335_forcar_standby(VI_PIPE ViPipe);
extern void IMX335_forcar_restart(VI_PIPE ViPipe);
extern int  IMX335_forcar_write_register(VI_PIPE ViPipe, int addr, int data);
extern int  IMX335_forcar_read_register(VI_PIPE ViPipe, int addr);
...
#define IMX335_FRAME_TYPE_SHORT      1
#define IMX335_FRAME_TYPE_LONG       0
...
static HI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    IMX335_FORCAR_SENSOR_GET_CTX(ViPipe, pstSnsState);
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->u8ImgMode)
    {
        ...
        case IMX335_4M_25FPS_10BIT_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 15))
            {
                ...
                u32Lines = u32Lines + (u32Lines % 2); //VMAX is 2n(n-0,1,2.....)
                pstSnsState->u32FLStd = u32Lines;
                //pstAeSnsDft->u32MaxIntTime = pstSnsState->u32FLStd - 7;
            }
            else
            {
                ISP_TRACE(HI_DBG_ERR, "Not support Fps: %f\n", f32Fps);
    ...
    {
        pstSnsState->astRegsInfo[0].astI2cData[5].u32Data = IMX335_FORCAR_LOW_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = IMX335_FORCAR_MID_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = IMX335_FORCAR_HIG_4BITS(pstSnsState->au32FL[0]);

    }
    else
    {
        pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = IMX335_FORCAR_LOW_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = IMX335_FORCAR_MID_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = IMX335_FORCAR_HIG_4BITS(pstSnsState->au32FL[0]);
    }
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        */
        g_astimx335_forcar_State[ViPipe].u32RHS1_MAX = g_astimx335_forcar_State[ViPipe].u32BRL * 2;
        g_astimx335_forcar_State[ViPipe].u32deltaRHS1 = (u32Lines * 2) - (g_astimx335_forcar_State[ViPipe].u32BRL * 2) - 2;
    }
    else
    {
        pstSnsState->u32FLStd = u32Lines;
...
static HI_VOID cmos_slow_framerate_set(VI_PIPE ViPipe, HI_U32 u32FullLines, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        //u32FullLines = (u32FullLines > (2 * IMX335_FULL_LINES_MAX)) ? (2 * IMX335_FULL_LINES_MAX) : u32FullLines;
        u32FullLines = (u32FullLines > (2*2*IMX335_VMAX_5M_30FPS_10BIT_WDR)) ? (2*2*IMX335_VMAX_5M_30FPS_10BIT_WDR) : u32FullLines;
        if (0 != (u32FullLines % 4))
        {
        ...
        g_astimx335_forcar_State[ViPipe].u32RHS1_MAX = g_astimx335_forcar_State[ViPipe].u32BRL * 2;
        g_astimx335_forcar_State[ViPipe].u32deltaRHS1 = u32FullLines - (g_astimx335_forcar_State[ViPipe].u32BRL * 2) - 2;
    }
    else
    {
    ...
    if (WDR_MODE_NONE == pstSnsState->enWDRMode)
    {
        pstSnsState->astRegsInfo[0].astI2cData[5].u32Data = IMX335_FORCAR_LOW_8BITS(pstSnsState->au32FL[0]);   //VMAX
        pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = IMX335_FORCAR_MID_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = IMX335_FORCAR_HIG_4BITS(pstSnsState->au32FL[0]);
    }
    else
    {
        pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = IMX335_FORCAR_LOW_8BITS(pstSnsState->au32FL[0]>>1);    //VMAX
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = IMX335_FORCAR_MID_8BITS(pstSnsState->au32FL[0]>>1);
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = IMX335_FORCAR_HIG_4BITS(pstSnsState->au32FL[0]>>1);
    }
...
{
    static HI_U8 u8ExpFrameType = IMX335_FRAME_TYPE_SHORT;
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        if(IMX335_FRAME_TYPE_SHORT == u8ExpFrameType)/* short exposure */
        {
            pstSnsState->au32WDRIntTime[0] = u32IntTime;
            u32ShortIntTime = u32IntTime;

            u8ExpFrameType = IMX335_FRAME_TYPE_LONG;
            //printf("\n\nst = %d\n", u32IntTime);
        }
        else/* long exposure */
        {
            ...
            //u32SHR0 = pstSnsState->au32FL[1] - u32LongIntTime + (u32LongIntTime % 4) - 4;
            u32SHR0 = pstSnsState->au32FL[1] - u32LongIntTime + (u32LongIntTime % 4);

#if 0
            /*Change Short Time and Short Gain*/
            //fps = 25, stime = 10ms
            //printf("u32STimeFps = %d\n", u32STimeFps);
            if (30 == gu32STimeFps)
            {
                if ((2700 <= u32LongIntTime) && (2700 > u32ShortIntTime) && (8720 > (u32LongIntTime + 2704)) && (72 <= gu32LGain))
                {
                    SGain = (ad_gain_table[gu32LGain] * u32ShortIntTime) / 2700;
                    for (i = 1; i < 141; i++)
                    {
                        if (SGain < ad_gain_table[i])
                        {
                            SGain = ad_gain_table[i - 1];
                            SGainDb = i - 1;
                            break;
                        }
                    }
                    u32ShortIntTime = 2700;
                    pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = IMX335_FORCAR_LOW_8BITS(SGainDb);
                    pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = IMX335_FORCAR_MID_8BITS(SGainDb);
                }
                else
                {
                    pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = IMX335_FORCAR_LOW_8BITS(gu32LGain);
                    pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = IMX335_FORCAR_MID_8BITS(gu32LGain);
                }
            }
            //fps = 30, stime = 8.4ms
            /*
            else if (30 == gu32STimeFps)
            {
                if ((2268 <= u32LongIntTime) && (2268 > u32ShortIntTime) && (8720 > (u32LongIntTime + 2272)) && (40 <= gu32LGain))
                {
                    SGain = (ad_gain_table[gu32LGain] * u32ShortIntTime) / 2268;
                    for (i = 1; i < 141; i++)
                    {
                        if (SGain < ad_gain_table[i])
                        {
            ...
            */
            ...
            if (u32LastShortIntTime > u32ShortIntTime)
            {
                ...
                u32RHS1Limit = (pstSnsState->au32FL[1] - (g_astimx335_forcar_State[ViPipe].u32BRL * 2) - 2);
                if (u32delta > u32RHS1Limit)
                {
                    //u32ShortIntTime = u32LastShortIntTime - (g_astimx335State[ViPipe].u32deltaRHS1 - (g_astimx335State[ViPipe].u32deltaRHS1 % 8));
                    u32ShortIntTime = u32LastShortIntTime - (u32RHS1Limit - (u32RHS1Limit % 8) + 8);
                }
            }
            //u32RHS1 = u32SHR1 + u32ShortIntTime -  (u32ShortIntTime % 8);
			...
			#if 0
            u32RHS1Limit = (((pstSnsState->au32FL[1] - g_astimx335State[ViPipe].u32BRL*2) /4 -2)*2 + 3)*2;

            if ((u32RHS1Pre > u32RHS1) && ((u32RHS1Pre - u32RHS1) > u32RHS1Limit))
            {
               u32RHS1 = (u32RHS1Limit - (u32RHS1Limit-2)%8);
            }
			#endif
            //VC wdr don't need change YOUTSIZE
            //u32YOUTSIZE = 1984 + (((u32RHS1 / 2) - 3) / 2);
            //u32YOUTSIZE = (u32YOUTSIZE >= 0x1FFF) ? 0x1FFF : u32YOUTSIZE;

            //printf("SHR1: %d, RHS1: %d, S-Dif: %d\n", u32SHR1, u32RHS1, u32RHS1 - u32SHR1);
            //printf("SHR0: %d, CurFSC: %d, PreFSC: %d, L-Dif: %d\n\n", u32SHR0, pstSnsState->au32FL[0], pstSnsState->au32FL[1], pstSnsState->au32FL[0]-u32SHR0);
            //printf("g_astimx335State[IspDev].u32deltaRHS1 = %d\n", g_astimx335State[IspDev].u32deltaRHS1);
            //printf("g_astimx335State[IspDev].u32BRL = %d\n", g_astimx335State[IspDev].u32BRL);
            //printf("g_apstSnsState[IspDev]->au32FL[1] = %d\n", g_apstSnsState[IspDev]->au32FL[1]);
            pstSnsState->astRegsInfo[0].astI2cData[0].u32Data =  IMX335_FORCAR_LOW_8BITS(u32SHR0);    //SHR0
            pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = IMX335_FORCAR_MID_8BITS(u32SHR0);
            pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = IMX335_FORCAR_HIG_4BITS(u32SHR0);

            pstSnsState->astRegsInfo[0].astI2cData[5].u32Data =  IMX335_FORCAR_LOW_8BITS(u32SHR1);    //SHR1
            pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = IMX335_FORCAR_MID_8BITS(u32SHR1);
            pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = IMX335_FORCAR_HIG_4BITS(u32SHR1);

            pstSnsState->astRegsInfo[0].astI2cData[11].u32Data =  IMX335_FORCAR_LOW_8BITS(u32RHS1);    //RHS1
            pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = IMX335_FORCAR_MID_8BITS(u32RHS1);
            pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = IMX335_FORCAR_HIG_4BITS(u32RHS1);

            //g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[14].u32Data = LOW_8BITS(u32YOUTSIZE);   //YOUTSIZE
            //g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[15].u32Data = MIDDLE_8BITS(u32YOUTSIZE);
            u8ExpFrameType= IMX335_FRAME_TYPE_SHORT;
    ...
    else  //Linear
    {
        ...
        //night antiflicker
        //printf("LGain = %d\n", LGain);
        if (2160 < u32IntTime && 2700 > u32IntTime && 70 < gu32LGain)
        {
            LinearExpTime = 2700;
            SGain = (ad_gain_table[gu32LGain] * u32IntTime) / 2700;
            for (i = 1; i < 141; i++)
            {
                if (SGain < ad_gain_table[i])
                {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[3].u32Data = IMX335_FORCAR_LOW_8BITS(SGainDb);
            pstSnsState->astRegsInfo[0].astI2cData[4].u32Data = IMX335_FORCAR_MID_8BITS(SGainDb);
            //printf("SGain = %d, SGainDb = %d\n", SGain, SGainDb);

        }
        else
        {
            LinearExpTime = u32IntTime;
        }
#endif
        ...
        pstSnsState->astRegsInfo[0].astI2cData[0].u32Data = IMX335_FORCAR_LOW_8BITS(u32Value);   //SHR0
        pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = IMX335_FORCAR_MID_8BITS(u32Value);
        pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = IMX335_FORCAR_HIG_4BITS(u32Value);
    }
...
{
    static HI_U8 u8ExpFrame = IMX335_FRAME_TYPE_LONG;
    HI_U32 u32Tmp;
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        if(IMX335_FRAME_TYPE_LONG == u8ExpFrame)
        {
            u32Tmp = u32Again + u32Dgain;
            gu32LGain = u32Tmp;

            pstSnsState->astRegsInfo[0].astI2cData[3].u32Data = IMX335_FORCAR_LOW_8BITS(u32Tmp);
            pstSnsState->astRegsInfo[0].astI2cData[4].u32Data = IMX335_FORCAR_MID_8BITS(u32Tmp);
			u8ExpFrame = IMX335_FRAME_TYPE_SHORT;
		}
		else
		{
            u32Tmp = u32Again + u32Dgain;

            pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = IMX335_FORCAR_LOW_8BITS(u32Tmp);
            pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = IMX335_FORCAR_MID_8BITS(u32Tmp);
			u8ExpFrame = IMX335_FRAME_TYPE_LONG;
		}
	}
	else
	{
...
static HI_VOID cmos_get_inttime_max(VI_PIPE ViPipe, HI_U16 u16ManRatioEnable, HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    if ((WDR_MODE_2To1_LINE == pstSnsState->enWDRMode))
    {
        ...
        */
        #if 0
        u32BRLCalTemp = g_astimx335_forcar_State[ViPipe].u32BRL * 2;
        g_astimx335_forcar_State[ViPipe].u32RHS1_MAX = (((pstSnsState->au32FL[0] - u32BRLCalTemp) / 4 - 2) * 2 + 3) * 2;

        if (g_astimx335_forcar_State[ViPipe].u32RHS1_MAX > u32BRLCalTemp)   //RHS1_MAX need less than BRL*2
        {
            g_astimx335_forcar_State[ViPipe].u32RHS1_MAX = u32BRLCalTemp;
        }
        #endif
        //printf("Lation@ cmos_get_inttime_max au32FL[0]: %d, RHS1_MAX:%d\n", pstSnsState->au32FL[0], g_astimx335State[ViPipe].u32RHS1_MAX);
        ...
        else if (ISP_FSWDR_AUTO_LONG_FRAME_MODE == genFSWDRMode[ViPipe])
        {
            ...
            else
            {
                ...
                u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
                u32IntTimeMaxTmp = (u32IntTimeMaxTmp > (g_astimx335_forcar_State[ViPipe].u32RHS1_MAX - 18)) ? (g_astimx335_forcar_State[ViPipe].u32RHS1_MAX - 18) : u32IntTimeMaxTmp;
                if (u32IntTimeMaxTmp > u32LastIntTimeMaxTmp)
                {
                    u32delta = u32IntTimeMaxTmp - u32LastIntTimeMaxTmp;
                    if (u32delta > g_astimx335_forcar_State[ViPipe].u32deltaRHS1)
                    {
                        u32IntTimeMaxTmp = u32LastIntTimeMaxTmp + (g_astimx335_forcar_State[ViPipe].u32deltaRHS1 - (g_astimx335_forcar_State[ViPipe].u32deltaRHS1 % 8));
                    }
                    else
                    {
                    }
        ...
        else
        {
            ...
            u32IntTimeMaxTmp = (0 == u32IntTimeMaxTmp) ? 1 : u32IntTimeMaxTmp;
            u32LastIntTimeMaxTmp = u32IntTimeMaxTmp;
			*pu32LFMaxIntTime = g_astimx335_forcar_State[ViPipe].u32RHS1_MAX - 18;
        }
    }
    else
    {
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    {
        IMX335_forcar_write_register(ViPipe, IMX335_GAIN_LONG_LOW, 0x00); //gain
        IMX335_forcar_write_register(ViPipe, IMX335_GAIN_LONG_HIGH, 0x00);

        IMX335_forcar_write_register(ViPipe, IMX335_VMAX_ADDR_L, IMX335_FORCAR_LOW_8BITS(u32FullLines_5Fps));    //VMAX
        IMX335_forcar_write_register(ViPipe, IMX335_VMAX_ADDR_M, IMX335_FORCAR_MID_8BITS(u32FullLines_5Fps));
        IMX335_forcar_write_register(ViPipe, IMX335_VMAX_ADDR_H, IMX335_FORCAR_HIG_4BITS(u32FullLines_5Fps));

        IMX335_forcar_write_register(ViPipe, IMX335_SHR0_LOW, IMX335_FORCAR_LOW_8BITS(u32MaxIntTime_5Fps));
        IMX335_forcar_write_register(ViPipe, IMX335_SHR0_MIDDLE, IMX335_FORCAR_MID_8BITS(u32MaxIntTime_5Fps));
        IMX335_forcar_write_register(ViPipe, IMX335_SHR0_MIDDLE, IMX335_FORCAR_HIG_4BITS(u32MaxIntTime_5Fps));
    }
    else /* setup for ISP 'normal mode' */
    {
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > IMX335_FULL_LINES_MAX) ? IMX335_FULL_LINES_MAX : pstSnsState->u32FLStd;
        IMX335_forcar_write_register (ViPipe, IMX335_VMAX_ADDR_L, IMX335_FORCAR_LOW_8BITS(pstSnsState->u32FLStd));
        IMX335_forcar_write_register (ViPipe, IMX335_VMAX_ADDR_M, IMX335_FORCAR_MID_8BITS(pstSnsState->u32FLStd));
        IMX335_forcar_write_register (ViPipe, IMX335_VMAX_ADDR_H, IMX335_FORCAR_HIG_4BITS(pstSnsState->u32FLStd));
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((HI_FALSE == pstSnsState->bSyncInit) || (HI_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx335_forcar_BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = imx335_forcar_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = imx335_forcar_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = imx335_forcar_data_byte;
        }
        ...
        //pstSnsState->astRegsInfo[0].astI2cData[2].u8IntPos = 1;
        //pstSnsState->astRegsInfo[0].astI2cData[3].u8IntPos = 1;
...
#define IMX335_FOR_CAR_ERR_MODE_PRINT(pstSensorImageMode,pstSnsState)\
    do{\
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        if (IMX335_RES_IS_4M_10BIT_WDR_EX(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
        {
            ...
            g_astimx335_forcar_State[ViPipe].u32BRL = 1480 * 2;
                 ...
                 IMX335_RES_IS_4M_10BIT_WDR(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
        {
            ...
            g_astimx335_forcar_State[ViPipe].u32BRL = 1984 * 2;
            pstSnsState->u32FLStd                   = IMX335_VMAX_5M_30FPS_10BIT_WDR;
        }
        else
        {
            IMX335_FOR_CAR_ERR_MODE_PRINT(pstSensorImageMode, pstSnsState);
            return HI_FAILURE;
        }
    }
...
static HI_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_sensor_init         = IMX335_forcar_init;
    pstSensorExpFunc->pfn_cmos_sensor_exit         = IMX335_forcar_exit;
...
static HI_S32 IMX335_forcar_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunImx335_forcar_BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static HI_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    IMX335_FORCAR_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    IMX335_FORCAR_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static HI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ...
    IMX335_FORCAR_SENSOR_RESET_CTX(ViPipe);
}
...
ISP_SNS_OBJ_S stSnsImx335Obj =
{
    ...
    .pfnStandby             = IMX335_forcar_standby,
    .pfnRestart             = IMX335_forcar_restart,
    .pfnMirrorFlip          = HI_NULL,
    .pfnWriteReg            = IMX335_forcar_write_register,
    .pfnReadReg             = IMX335_forcar_read_register,
    .pfnSetBusInfo          = IMX335_forcar_set_bus_info,
