...
static HI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (Os05a_5M_2688x1944_12bit_linear30 == pstSnsState->u8ImgMode)
    {
        u32Fll = OS05A_VMAX_5M30_12bit_LINEAR;
        f32maxFps = 30;
    }
    else if (Os05a_5M_2688x1944_10bit_wdr30 == pstSnsState->u8ImgMode)
    {
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        {
            //printf("cmos_inttime_update: long exposure\n");
            pstSnsState->au32WDRIntTime[1] = u32IntTime;
            u32LongIntTime = u32IntTime;
            if (30 == gu32STimeFps)
            {
                //printf("cmos_inttime_update: u32ShortIntTime = %d\n", u32ShortIntTime);
                //printf("cmos_inttime_update: gf32AGain = %f\n", gf32AGain);
                //printf("cmos_inttime_update: gf32DGain = %f\n", gf32DGain);
                if ((750 <= u32LongIntTime) && (750 > u32ShortIntTime) && (2450 > (u32LongIntTime + 750)) && (11 <= gf32AGain))
                {
                    fCGain = gf32AGain * gf32DGain * u32ShortIntTime / 750;
                    //printf("cmos_inttime_update: fCGain = %f\n", fCGain);
                    if (fCGain < 16)
                    {
                        u32Again = fCGain * 128;
                        u32Dgain = 1024;
                    ...
                    //printf("cmos_inttime_update: u32Again = %d, u32Dgain = %d\n", u32Again, u32Dgain);
                    u32ShortIntTime = 750;
...
static HI_VOID cmos_gains_update(VI_PIPE ViPipe, HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    //printf("cmos_gains_update: AGain = %d, DGain = %d\n", u32Again*8, u32Dgain);
    //printf("cmos_gains_update: gf32AGain = %f, gf32DGain = %f\n", gf32AGain, gf32DGain);

    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        if(OS05A_FRAME_TYPE_LONG == u8ExpFrame)
        {
        ...
        else if(OS05A_FRAME_TYPE_SHORT == u8ExpFrame)
        {
            pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = LOW_8BITS(u32Again);
            pstSnsState->astRegsInfo[0].astI2cData[11].u32Data = ((u32Again & 0x3F00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = LOW_8BITS(u32Dgain);
