...
HI_S32 HI_MPI_ISP_SetStatisticsConfig(VI_PIPE ViPipe, const ISP_STATISTICS_CFG_S *pstStatCfg)
{
    ...
    if (ISP_MODE_RAW == stIspWorkMode.enIspDataInputMode) {
        ...
        if ((ISP_AE_FOUR_PLANE_MODE_DISABLE == pstStatCfg->stAECfg.enFourPlaneMode) &&
		    (ISP_AE_HIST_SKIP_EVERY_PIXEL == pstStatCfg->stAECfg.stHistConfig.enHistSkipX)) {
...
HI_S32 HI_MPI_ISP_SetDPCalibrate(VI_PIPE ViPipe, const ISP_DP_STATIC_CALIBRATE_S *pstDPCalibrate)
{
    ...
    if (((ISP_MODE_RUNNING_OFFLINE == stIspWorkMode.enIspRunningMode) ||
	     (ISP_MODE_RUNNING_STRIPING == stIspWorkMode.enIspRunningMode)) &&
        (pstDPCalibrate->bEnableDetect == HI_TRUE) &&
		(1 != ISP_SUPPORT_OFFLINE_DPC_CALIBRATION)) {
