...
HI_S32 SHA224(void)
{
    ...
    for(i = 0; i < MAX_HASH_HANDLE; i++)
    {
     ...
     for(i = 0; i < MAX_HASH_HANDLE; i++)
     {
        ...
        {
            HI_ERR_CIPHER("SHA224 run failed, sample %d!\n", i);
...
int sample_hash(int argc, char* argv[])
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("SHA256 run failed!\n");
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("HMAC_SHA1 run failed!\n");
        return s32Ret;
    }
    s32Ret = HMAC_SHA256();
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("HMAC_SHA256 run failed!\n");
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("SHA224 run failed!\n");
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("SHA384 run failed!\n");
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("SHA512 run failed!\n");
        return s32Ret;
    }
#endif
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_CIPHER("SM3 run failed!\n");
        return s32Ret;
    }
#else
        HI_INFO_CIPHER("Sm3 is unsupported.\n");
#endif
