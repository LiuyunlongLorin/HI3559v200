...
static HI_S32 ECB_AES256(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32TestDataLen = 32;
    HI_SIZE_T szInputAddrPhy = 0;
    HI_SIZE_T szOutPutAddrPhy = 0;
    ...
    HI_U8 aes_key[32] = {0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,
                         0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11};
    HI_U8 aes_src[32] = {0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,
                         0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x11};
    HI_U8 aes_dst[32] = {0x86,0x3a,0xe9,0x1f,0xd4,0xcf,0x51,0x13,0x2d,0xc3,0xe4,0xed,0xf0,0x9d,0xbc,0x6e,
    ...
    szInputAddrPhy = HI_MMZ_New(u32TestDataLen, 0, NULL, "CIPHER_BufIn");
    if (0 == szInputAddrPhy)
    {
        HI_ERR_CIPHER("Error: Get phyaddr for input failed!\n");
        goto __CIPHER_EXIT__;
    }
    pInputAddrVir = HI_MMZ_Map(szInputAddrPhy, u32TestDataLen, u32Testcached);
    szOutPutAddrPhy = HI_MMZ_New(u32TestDataLen, 0, NULL, "CIPHER_BufOut");
    if (0 == szOutPutAddrPhy)
    {
        HI_ERR_CIPHER("Error: Get phyaddr for outPut failed!\n");
        goto __CIPHER_EXIT__;
    }
    pOutputAddrVir = HI_MMZ_Map(szOutPutAddrPhy, u32TestDataLen, u32Testcached);
    ...
    s32Ret = HI_UNF_CIPHER_Encrypt(hTestchnid, szInputAddrPhy, szOutPutAddrPhy, u32TestDataLen);
    if(HI_SUCCESS != s32Ret)
    {
   ...
   /* For decrypt */
    ...
    s32Ret = HI_UNF_CIPHER_Decrypt(hTestchnid, szInputAddrPhy, szOutPutAddrPhy, u32TestDataLen);
    if(HI_SUCCESS != s32Ret)
    {
...
__CIPHER_EXIT__:
    ...
    if (szInputAddrPhy> 0)
    {
        HI_MMZ_Unmap(szInputAddrPhy);
        HI_MMZ_Delete(szInputAddrPhy);
    }
    if (szOutPutAddrPhy > 0)
    {
        HI_MMZ_Unmap(szOutPutAddrPhy);
        HI_MMZ_Delete(szOutPutAddrPhy);
    }
...
/* encrypt data using special chn*/
static HI_S32 CBC_AES256(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
*/
    ...
    HI_U8 aes_dst[16] = {0xe0,0xfa,0xab,0x42,0xd5,0xa5,0x5c,0x18,0xb0,0x13,0x1f,0x47,0x6c,0xa5,0xd0,0x0a};
...
static HI_S32 CBC_AES192(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32TestDataLen = 16;
    HI_SIZE_T szInputAddrPhy = 0;
...
*/
    ...
    HI_U8 aes_IV[16]  = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F};
    HI_U8 aes_src[16] = {0x6B,0xC1,0xBE,0xE2,0x2E,0x40,0x9F,0x96,0xE9,0x3D,0x7E,0x11,0x73,0x93,0x17,0x2A};
    HI_U8 aes_dst[16] = {0x69,0x25,0x8f,0xc2,0x8d,0x58,0x8b,0x84,0x67,0x87,0x76,0x02,0x2f,0x9c,0x1c,0x91};
...
#if defined(CHIP_TYPE_hi3559av100) || defined(CHIP_TYPE_hi3519av100) || defined(CHIP_TYPE_hi3556av100) || defined(CHIP_TYPE_hi3559av100es)
...
HI_S32 CCM_AES128(HI_VOID)
{
    ...
    szInputAddrPhy = HI_MMZ_New(32, 0, NULL, "CIPHER_BufIn");
    if (0 == szInputAddrPhy)
    {
        HI_ERR_CIPHER("Error: Get phyaddr for input failed!\n");
        goto __CIPHER_EXIT__;
    ...
    pInputAddrVir = HI_MMZ_Map(szInputAddrPhy, u32TestDataLen, u32Testcached);
    szOutPutAddrPhy = HI_MMZ_New(32, 0, NULL, "CIPHER_BufOut");
    if (0 == szOutPutAddrPhy)
    {
        HI_ERR_CIPHER("Error: Get phyaddr for outPut failed!\n");
        goto __CIPHER_EXIT__;
    ...
    szAadAddrPhy = HI_MMZ_New(sizeof(aes_a), 0, NULL, "CIPHER_AAD");
    if (0 == szAadAddrPhy)
    {
        HI_ERR_CIPHER("Error: Get phyaddr for AAD failed!\n");
        goto __CIPHER_EXIT__;
    }
    pAadAddrVir = HI_MMZ_Map(szAadAddrPhy, u32TestDataLen, u32Testcached);
    ...
    stCtrl.szAPhyAddr = szAadAddrPhy;
...
__CIPHER_EXIT__:
    ...
    if (szAadAddrPhy > 0)
    {
        HI_MMZ_Unmap(szAadAddrPhy);
        HI_MMZ_Delete(szAadAddrPhy);
    }
...
static HI_S32 CCM_AES128_2(HI_VOID)
{
    ...
    pInputAddrVir = HI_MMZ_Map(szInputAddrPhy, u32TestDataLen, 0);
    ...
    pOutputAddrVir = HI_MMZ_Map(szOutPutAddrPhy, u32TestDataLen, 0);
    ...
    pAadAddrVir = HI_MMZ_Map(szAadAddrPhy, u32TestDataLen, 0);
    ...
    printBuffer("CCM-AES-128-ORITAG:", aes_tag, u32TagLen);
    printBuffer("CCM-AES-128-GETTAG", out_tag, sizeof(aes_tag));
