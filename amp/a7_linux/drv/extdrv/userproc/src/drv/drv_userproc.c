...
#define HI_FATAL_UPROC(fmt, ...)   printk("<%d>Func:%s, Line:%d, "fmt"\n", HI_TRACE_LEVEL_FATAL, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define HI_ERR_UPROC(fmt, ...)     printk("<%d>Func:%s, Line:%d, "fmt"\n", HI_TRACE_LEVEL_ERROR, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define HI_WARN_UPROC(fmt, ...)    //printk("<%d>Func:%s, Line:%d, "fmt"\n", HI_TRACE_LEVEL_WARN, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define HI_INFO_UPROC(fmt, ...)    //printk("<%d>Func:%s, Line:%d, "fmt"\n", HI_TRACE_LEVEL_INFO, __FUNCTION__, __LINE__, ##__VA_ARGS__)
...
static UMP_ENTRY_S * RBTree_Find_Proc_Entry(struct proc_dir_entry* pstPde)
{
    ...
    if(!pstEntry)
    {
        HI_ERR_UPROC("Can't find entry:%s\n", pstPde->name);
        goto out;
    }
...
#define DEFAULT_ECHO_DEVICE_HANDLE (0)
    ...
    if (ret)
    {
        HI_ERR_UPROC("Default echo device handle(%u) invalid!\n", DEFAULT_ECHO_DEVICE_HANDLE);
        return;
    }
    ...
    if ( S_ISCHR(stat.mode) && (MAJOR(stat.rdev) == TTYAUX_MAJOR || MAJOR(stat.rdev) == UNIX98_PTY_SLAVE_MAJOR) )
    {
        ...
        if (ret < 0)
        {
            HI_ERR_UPROC("write to echo device failed(%d)!\n", ret);
        }
        set_fs(stOld_FS);
    ...
    {
        HI_ERR_UPROC("Default echo device is invalid!\n");
    }
...
HI_VOID HI_DRV_PROC_EchoHelper(const HI_CHAR *fmt, ...)
{
    ...
    if (!fmt)
    {
        HI_ERR_UPROC("Invalid argument fmt!\n");
        return;
    }
    pucBuf = kmalloc(HI_PROC_BUFFER_SIZE, GFP_KERNEL);
    if (HI_NULL == pucBuf)
    {
        HI_ERR_UPROC("Memory allocate failed for proc\n");
        return;
    }
...
static int ump_seq_show(struct seq_file *m, void *unused)
{
    ...
    strncpy(proc->current_cmd.aszCmd, HI_UPROC_READ_CMD, sizeof(proc->current_cmd.aszCmd) - 1);
    ...
    if (pstEntry->is_removing)
    {
        HI_ERR_UPROC("entry:%s is removing\n", pstPde->name);
        ret = -1;
        return ret;
    }
...
static ssize_t ump_seq_write(struct file *file, const char __user *buf, size_t size, loff_t *pos)
{
    ...
    pstEntry = RBTree_Find_Proc_Entry(pstPde);
    if (pstEntry && (pstEntry->is_removing))
    {
        HI_ERR_UPROC("entry:%s is removing\n", pstPde->name);
        ret = -1;
        goto out;
...
HI_VOID PROC_RemoveEntry(UMP_DIR_S *pstDir, UMP_ENTRY_S * pstEntry)
{
    ...
    pstEntry->is_removing = 1;
    wake_up_interruptible(&g_stUProcParam.wq_for_read);
    wake_up_interruptible(&g_stUProcParam.wq_for_write);
    /* Remove proc entry */
    remove_proc_entry(pstEntry->entry_name, pstEntry->parent);
...
module_init(USRPROC_DRV_ModInit);
module_exit(USRPROC_DRV_ModExit);
