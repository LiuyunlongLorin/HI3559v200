...
static USERPROC_PARAM_S g_stUprocParam =
{
    ...
    .stRemoveMutex = PTHREAD_MUTEX_INITIALIZER,
    .apu8Cmd = {
...
HI_S32 MPI_UPROC_AddEntry(HI_U32 u32ModuleID, const HI_PROC_ENTRY_S* pstEntry)
{
    ...
    strncpy(stEntry.aszName, pstEntry->pszEntryName, sizeof(stEntry.aszName)-1);
    if (HI_NULL != pstEntry->pszDirectory)
    {
        strncpy(stEntry.aszParent, pstEntry->pszDirectory, sizeof(stEntry.aszParent)-1);
    }
...
HI_S32 MPI_UPROC_RemoveEntry(HI_U32 u32ModuleID, const HI_PROC_ENTRY_S* pstEntry)
{
    ...
    (HI_VOID)pthread_mutex_lock(&g_stUprocParam.stRemoveMutex);
    memset(&stEntry, 0, sizeof(stEntry));
    strncpy(stEntry.aszName, pstEntry->pszEntryName, sizeof(stEntry.aszName)-1);
    if (HI_NULL != pstEntry->pszDirectory)
    {
	...
	(HI_VOID)pthread_mutex_unlock(&g_stUprocParam.stRemoveMutex);
...
static HI_VOID* MPI_UPROC_Thread(HI_VOID* pParam)
{
    ...
    stBuffer.pu8Buf = (HI_U8*)malloc(HI_PROC_BUFFER_SIZE);
    ...
    prctl(PR_SET_NAME, (unsigned long) "Hi_pTUproc", 0, 0, 0);
    while (!g_stUprocParam.bThreadStop)
    {
        ...
        if (strlen(stCMD.stCmd.aszCmd) > 0)
        {
            HI_INFO_UPROC("proc GetCMD success: %s.\n", stCMD.stCmd.aszCmd);
                ...
                /* read */
                if (0 == strncmp(HI_UPROC_READ_CMD, stCMD.stCmd.aszCmd, strlen(HI_UPROC_READ_CMD)+1))
                {
	...
	free(stBuffer.pu8Buf);
	stBuffer.pu8Buf= HI_NULL;
...
{
    return MPI_UPROC_Init();
}

HI_S32 HI_PROC_DeInit(HI_VOID)
{
    return MPI_UPROC_DeInit();
}
