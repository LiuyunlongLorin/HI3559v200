...
#else
        writereg(0x0 << 21,0x7 << 21,PERI_CRG40);/**<[23,21] vpss clk,000 475M*/
        writereg(0x2 << 15,0x7 << 15,PERI_CRG40);/**<[17,15] ive clk,010 300M*/
        writereg(0x0 << 6,0x7 << 6,PERI_CRG40);/**[8,6] vgs clk,000 475M*/
        writereg(0x0 << 4,0x3 << 4,PERI_CRG40);/**[5,4] gdc clk,00 475M*/
...
{
#if (defined(CONFIG_SNS0_IMX335) && defined(CONFIG_SNS1_BT656_1080P25) && defined(HI3556V200))
    /**do not need GDC, return*/
    return HI_SUCCESS;
#endif
...
HI_S32 HI_PDT_INIT_CHIP_PreInit(HI_VOID)
{
...
#endif
#ifdef CONFIG_SNS0_OS04B10
    g_stSensorCfg[0].enSensortype = PDT_INIT_SENSOR_TYPE_I2C;
    g_stSensorCfg[0].u32SensorBusId = 0;
    g_stSensorCfg[0].enSensorClk = PDT_INIT_SENSOR_CLK_24MHz;
    snprintf(g_stSensorCfg[0].szSensorName, MAX_SENSOR_NAME_LEN, "%s", "os04b10");
#endif
    ...
    sensor_driver_load();
    sensor_clock_config();
    HI_SYSTEM_SetSystemPwrEnable(HI_TRUE);
    return s32Ret;
}
