...
static HI_S32 PDT_UI_ReadTouchData(HIGV_TOUCH_INPUTINFO_S* info)
{
...
#else
    s_s32ScreenWidth = XML_LAYOUT_WIDTH;
    s_s32ScreenHeight = XML_LAYOUT_HEIGHT;
#endif
    }
    ...
    {
        info->x =  stHalInputInfo.s32Y * XML_LAYOUT_WIDTH / s_s32ScreenHeight;
        /** info->y = (s_s32ScreenWidth - stHalInputInfo.s32X) * XML_LAYOUT_HEIGHT / s_s32ScreenWidth; */


        info->y = XML_LAYOUT_HEIGHT - stHalInputInfo.s32X * XML_LAYOUT_HEIGHT / s_s32ScreenWidth;
        //info->y = stHalInputInfo.s32X * XML_LAYOUT_HEIGHT / s_s32ScreenWidth;

        if(XML_LAYOUT_HEIGHT == s_s32ScreenWidth && XML_LAYOUT_HEIGHT == info->y)
        {
            info->y -= 1;
        }
    }
    else if(HIGV_ROTATE_NONE== enRotate)
    {
        ...
        info->x =  XML_LAYOUT_HEIGHT - stHalInputInfo.s32X * XML_LAYOUT_HEIGHT / s_s32ScreenHeight;
        info->y =  stHalInputInfo.s32Y * XML_LAYOUT_WIDTH / s_s32ScreenWidth;

        if(XML_LAYOUT_WIDTH == s_s32ScreenWidth && XML_LAYOUT_WIDTH == info->y)
        {
            info->y -= 1;
        }
    }
