    ...
    HI_S32  skip_badblock)
{
    ...
    if ((HI_UL)offset & nandraw_ctrl->pagemask)
    {
        ...
        while ((ptn->start <= offset) && (offset < ptn->end) && length && (ptn->perm & ACCESS_RD) && ptn->fd != INVALID_FD)
        {
            ...
            if (skip_badblock)
            {
                ...
                if (rel > 0)
                {
                    ...
                    offset += (HI_UL)((HI_U32)rel << nandraw_ctrl->blockshift);
                    continue;
                }
            ...
            do
            {
                ...
                if (read_oob)
                {
                    ...
                    {
                        oob.start  = (HI_UL)(offset - ptn->start);
    ...
    HI_U64 limit_leng)
{
    ...
    if (((HI_UL)offset & nandraw_ctrl->blockmask)
        || ((HI_UL)length & nandraw_ctrl->blockmask))
    {
    ...
    for (ix = 0; ix < nandraw_ctrl->num_partition && length; ix++)
    {
        ...
        while ((ptn->start <= offset) && (offset < ptn->end) && length && (ptn->perm & ACCESS_WR) && ptn->fd != INVALID_FD)
        {
            ...
            do
            {
                ...
                write_req_buf.usr_data = (HI_U64)(HI_UL)data_ptr;
                write_req_buf.len = datalen;
                write_req_buf.usr_oob = (HI_U64)(HI_UL)oob_ptr;
...
HI_S32 nand_mark_badblock(HI_U64 offset, HI_U64 length)
{
    ...
    for (ix = 0; ix < nandraw_ctrl->num_partition && length; ix++)
    {
        ...
        while ((ptn->start <= offset) && (offset < ptn->end) && length && ptn->fd != INVALID_FD)
        {
            ...
            offset += (HI_UL)(1UL << nandraw_ctrl->blockshift);
            length -= (HI_UL)(1UL << nandraw_ctrl->blockshift);
        }
    }
    return 0;
}
        ...
        while ((ptn->start <= offset) && (offset < ptn->end) && length && ptn->fd != INVALID_FD)
        {
            ...
            if (badblock == 1)
            {
                HI_PRINT("Bad block at address: 0x%lX of \"%s\", absolute address: 0x%llX\n",
                         (HI_UL)blockoffset, ptn->mtddev, (HI_U64)offset);
            }
...
HI_S32 nand_raw_info(struct mtd_info_user* mtdinfo)
{
    ...
    {
        HI_PRINT("nand flash size out of range of an HI_UL.\n");
    }
    mtdinfo->size = (HI_UL)nandraw_ctrl->size;
