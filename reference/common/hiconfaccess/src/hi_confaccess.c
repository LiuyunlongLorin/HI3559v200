...
static HI_S32 CONFACCESS_Init(const HI_CHAR* pszCfgName, const HI_CHAR* pszCfgPath)
{
    ...
    if (NULL == pstCfgPos->stModule.pstIniDir)
    {
        ...
        HI_APPCOMM_SAFE_FREE(pstCfgPos);
        return HI_FAILURE;
    }
    ...
    for (s32Idx = 0; s32Idx < s32ModuleNum; ++s32Idx)
    {
        ...
        {
            MLOGE("Load %s failed\n", szModuleNodeKey);
            HI_APPCOMM_SAFE_FREE(pstModuleInfo);
            continue;
        }
        ...
        {
            strncpy(szCfgPath, pszCfgPath, strlen(pszCfgPath));
            dirname(szCfgPath);
            snprintf(pstModuleInfo->szPath, HI_CONFACCESS_PATH_MAX_LEN, "%s/", szCfgPath);
            MLOGI("Don't use certain module path, use default path[%s]\n", pstModuleInfo->szPath);
...
{
    HI_APPCOMM_CHECK_POINTER(pszCfgName, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_POINTER(pszCfgPath, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszCfgName) < HI_CONFACCESS_NAME_MAX_LEN, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszCfgPath) < HI_CONFACCESS_PATH_MAX_LEN, HI_CONFACCESS_EINVAL);
    ...
    s32Ret = CONFACCESS_Init(pszCfgName, pszCfgPath);
    HI_APPCOMM_CHECK_RETURN_WITH_ERRINFO(s32Ret, s32Ret, "AddCfg");
    return HI_SUCCESS;
}
...
HI_S32 HI_CONFACCESS_Deinit(const HI_CHAR* pszCfgName)
{
    HI_APPCOMM_CHECK_POINTER(pszCfgName, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszCfgName) < HI_CONFACCESS_NAME_MAX_LEN, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(s_s32CONFACCESSCount > 0, HI_CONFACCESS_ENOTINIT);
...
{
    HI_APPCOMM_CHECK_POINTER(pszCfgName, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_POINTER(pszModule, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_POINTER(pszConfItem, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_POINTER(ppszValue, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszCfgName) < HI_CONFACCESS_NAME_MAX_LEN, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszModule) < HI_CONFACCESS_NAME_MAX_LEN, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszConfItem) < HI_CONFACCESS_KEY_MAX_LEN, HI_CONFACCESS_EINVAL);
            ...
            const HI_CHAR* pszConfItem, const HI_CHAR* pszValue)
{
    ...
    HI_APPCOMM_CHECK_POINTER(pszModule, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_POINTER(pszConfItem, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_POINTER(pszValue, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszCfgName) < HI_CONFACCESS_NAME_MAX_LEN, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszModule) < HI_CONFACCESS_NAME_MAX_LEN, HI_CONFACCESS_EINVAL);
    HI_APPCOMM_CHECK_EXPR(strlen(pszConfItem) < HI_CONFACCESS_KEY_MAX_LEN, HI_CONFACCESS_EINVAL);
