...
HI_S32 HI_PDT_PARAM_Init(HI_VOID)
{
    ...
    MLOGD("Address[%x] ParamSize[%lu]\n", PDT_PARAM_PHY_ADDR, (HI_UL)sizeof(PDT_PARAM_CFG_S));
    s_stPARAMCtx.pstCfg = (PDT_PARAM_CFG_S *)PDT_PARAM_PHY_ADDR;
    MLOGD("MagicStart[%x] End[%x]\n", s_stPARAMCtx.pstCfg->stHead.u32MagicStart,
          s_stPARAMCtx.pstCfg->u32MagicEnd);
    ...
    MLOGD("PoweronAction[%d] PoweronWorkmode[%d]\n",
          s_stPARAMCtx.pstCfg->stWorkModeCfg.stCommCfg.enPoweronAction,
          s_stPARAMCtx.pstCfg->stWorkModeCfg.stCommCfg.enPoweronWorkMode);
#endif
