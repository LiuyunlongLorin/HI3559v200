...
/* Return a 32-bit CRC of the contents of the buffer accumulating the
   ...
   algorithm.*/
UINT32
ether_crc32_accumulate(UINT32 crc32val, UINT8 *src, INT32 length)
{
  INT32 c = 0;
  ...
  for (c = 0;  c < length;  c++) {
      crc32val = crc32_tab[(crc32val ^ src[c]) & 0xff] ^ (crc32val >> 8);
  }
...
unsigned int
crc32_accumulate(unsigned int crc32val, unsigned char *src, int length)
{
  INT32 c = 0;
...
UINT32
ether_crc32(UINT8 *src, INT32 length)
{
  return ether_crc32_accumulate(0,src,length);
}

/* This is the standard Gary S. Brown's 32 bit CRC algorithm */
UINT32
crc_crc32(UINT8 *src, INT32 length)
{
  return (crc32_accumulate(0,src,length));
}
