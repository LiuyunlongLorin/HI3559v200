...
print_button (WINDOW * win, const char *label, int y, int x, int selected)
{
    ...
    wmove (win, y, x);
    (void)wattrset (win, selected ? button_active_attr : button_inactive_attr);
    waddstr (win, "<");
    temp = strspn (label, " ");
    label += temp;
    (void)wattrset (win, selected ? button_label_active_attr
	      : button_label_inactive_attr);
    for (i = 0; i < temp; i++)
	waddch (win, ' ');
    (void)wattrset (win, selected ? button_key_active_attr
