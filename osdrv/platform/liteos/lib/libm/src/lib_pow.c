...
__ieee754_pow(double x, double y)
{
    ...
    double y1,t1,t2,r,s,t,u,v,w; /*lint !e578*/
    int i,j,k,yisint,n;
    int hx,hy,ix,iy;
    unsigned int lx,ly;
    ...
    if(iy>0x41e00000) { /* if |y| > 2**31 */
        ...
        if(ix<=0x3fefffff) return (hy<0)? huges*huges:tiny*tiny;
        if(ix>=0x3ff00000) return (hy>0)? huges*huges:tiny*tiny;
        }
    /* over/underflow if x is not close to one */
        if(ix<0x3fefffff) return (hy<0)? s*huges*huges:s*tiny*tiny;
        if(ix>0x3ff00000) return (hy>0)? s*huges*huges:s*tiny*tiny;
    ...
    if (j>=0x40900000) {                /* z >= 1024 */
        ...
        return s*huges*huges;            /* overflow */
        else {
        if(p_l+ovt>z-p_h) return s*huges*huges;    /* overflow */
        }
